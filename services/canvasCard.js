...
async function renderCard({ username, avatarUrl, fallbackAvatarUrl, rating, ratingCount, vouches }) {
  ...
  const ratingStartX = contentEndX - ratingBlockWidth;
  ...
  ctx.textAlign = 'left';
  ctx.textBaseline = 'middle';
  ctx.fillText(ratingLabel, ratingStartX, centerY);

  const starsStartX = ratingStartX + calcLabelWidth() + labelPadding;
  drawStarsStrongSlightlyRounded(ctx, starsStartX, centerY, 5, starSize, rating, starGap);

  const usernameAreaWidth = ratingStartX - contentStartX - 20;
  ...
  const vouchesText = `Vouches +${vouches}`;
  ...
  let vouchesTextWidth = ctx.measureText(vouchesText).width;
  ...
  while (vouchesBoxWidth > usernameAreaWidth && vouchesSize > minVouchesSize) {
    vouchesSize -= 1;
    ctx.font = `bold ${vouchesSize}px Arial`;
    vouchesTextWidth = ctx.measureText(vouchesText).width;
  ...
  ctx.textBaseline = 'middle';
  ctx.font = `bold ${vouchesSize}px Arial`;
  ctx.fillText(vouchesText, contentStartX + vouchesBoxWidth / 2, finalVouchesY + vouchesBoxHeight / 2);
  ...
  const middlemanText = 'middleman';
  ctx.font = `bold ${badgeFont}px Arial`;
  const middlemanTextWidth = ctx.measureText(middlemanText).width;
  const middlemanBadgeWidth = middlemanTextWidth + badgePadX * 2;
  const middlemanBadgeX = discordBadgeX - badgeGap - middlemanBadgeWidth;
  ctx.fillStyle = 'rgba(29,161,242,0.15)';
  ...
  ctx.fillText(middlemanText, middlemanBadgeX + middlemanBadgeWidth / 2, badgeBaseY + badgeHeight / 2);
  ...
  if (ratingCount) {
    ...
    ctx.fillText(reviewsLabel, ratingStartX, centerY + starSize + 16);
  }

  const buffer = canvas.toBuffer('image/png');
  return new AttachmentBuilder(buffer, { name: 'middleman_card.png' });
}
