...
export class WarnService {
  ...
  async #getWarnById(guildId, warnId) {
    if (!this.enabled) return null;
    this.logger.debug(`[WARN] Buscando warn ${warnId} en ${guildId}.`);
    const [[row]] = await this.db.execute(
      `SELECT id, guild_id, user_id, moderator_id, reason, points, context_message_url, created_at
       FROM warns
  ...
  async removeWarnById({ guildId, warnId }) {
    if (!this.enabled) return null;
    const warn = await this.#getWarnById(guildId, warnId);
    if (!warn) {
      return null;
    }
    this.logger.info(
      `[WARN] Eliminando warn ${warnId} de ${warn.user_id} en guild ${guildId} por solicitud administrativa.`
    );
    await this.db.execute(`DELETE FROM warns WHERE id = ? AND guild_id = ?`, [warnId, guildId]);
    return warn;
  }

  async removeLatestWarn({ guildId, userId }) {
    if (!this.enabled) return null;
    this.logger.debug(`[WARN] Buscando último warn de ${userId} en ${guildId}.`);
    const [[row]] = await this.db.execute(
      `SELECT id
       FROM warns
       WHERE guild_id = ? AND user_id = ?
    ...
    return this.removeWarnById({ guildId, warnId: row.id });
  }
}
